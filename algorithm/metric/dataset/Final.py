...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(testdata_loader):
            ...
            score1 = torch.pow(
                torch.sum(torch.pow(input_x_test[:, window_size - step_size:window_size, :] - test_rebuild, 2), dim=2),
                0.5)
            threshold1.append(score1)
            test_y.append(input_y_test[:, window_size - step_size:window_size, :])
    threshold1 = torch.cat(threshold1)
    # 评分的最大值最小值
    min_score = torch.min(threshold1)
    print("min_score:", min_score)
    max_score = torch.max(threshold1)
    print("max_score:", max_score)
    test_y = torch.cat(test_y).squeeze(2)

    thresholds_arr = [0.3, 0.2, 0.1, 0.08, 0.05, 0.02, 0.015, 0.01, 0.005, 0.002, 0.001, 0.0005, 0.0002, 0.0001]
    thresholds_setting = np.percentile(threshold1.cpu().detach().numpy(), 100 * (
            1 - np.array(thresholds_arr)))
    ...
    threshold_percent = thresholds_arr[np.where(acc == max(acc))[0][0]]
    # 判断是否存在异常的阈值
    final_threshold = (min_score + (max_score - min_score) * (1 - threshold_percent))
    print("score_threshold:", str(final_threshold))
    ...
    # 预测结果
    real_x = scio.loadmat('real_data')['machine1']
    real_x_windows = real_x[np.newaxis, :, :]
    dataset = MyDataset(real_x_windows, real_x_windows)
    real_data_loader = DataLoader(dataset, 1, shuffle=True, drop_last=True)
    with torch.no_grad():
        for batch in real_data_loader:
            ...
            real_score = torch.pow(torch.sum(torch.pow(input_x_real[:, window_size - step_size:window_size, :] - real_rebuild, 2), dim=2), 0.5)
            print(str(real_score))
