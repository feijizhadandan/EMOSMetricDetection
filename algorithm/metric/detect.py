...
def detectOnline(threadName: str, detectParam: dict, detectionTask: DetectionTask):
    ...
    # 搜集预测的数据
    metricMap = PROMETHEUS.getMetrics(load_metricNames, detectPodList, startTime=detectionTask.startTime.timestamp(), endTime=detectionTask.endTime.timestamp())
    PROMETHEUS.fillMetricsWithPreviousValue(metricMap)
    for podName in detectPodList:
        real_x = []
